...
public class UploadSpreadsheetPageController {
	...
	public FileDownload post(
			@RequestParam("spreadsheet") MultipartFile spreadsheetFile,
			UiUtils ui) {
		...
		String delimiter = ",";
		linesWithErrors.add("error reason" + delimiter + "map type" + delimiter
				+ "source name" + delimiter + "source code" + delimiter
				+ "concept Id" + delimiter + "concept uuid" + delimiter
				+ "preferred name" + delimiter + "description" + delimiter
				+ "class" + delimiter + "datatype" + delimiter
				+ "all existing mappings" + "\n");

		String filename = spreadsheetFile.getOriginalFilename();
		String[] nameWithoutExtension = filename.split(".csv");
		String sourceId = nameWithoutExtension[0]
				.substring(nameWithoutExtension[0].length() - 1);

		try {

			br = new BufferedReader(new InputStreamReader(
					spreadsheetFile.getInputStream()));
			int columnNumber = 1;
			boolean header = true;
					...
					for (String t : tokens) {
						...
						case 1:
							...
							if (t.replace("\"", "").trim().length() <= 1
								...
								errorString = " "
										+ errorString
										+ ui.message("conceptmanagementapps.file.maptype.error")
										+ " ";
							} else {
								try {
									conceptMap
											.setConceptMapType(cs
													.getConceptMapTypeByName(mapTypeName));
						...
						case 3:
							...
							source = cs.getConceptSource(Integer
									.valueOf(sourceId.trim()));
							if (t.replace("\"", "").trim().length() <= 1
									|| cs.getConceptReferenceTermByCode(t,
											source) == null) {
								errorString = " "
										+ errorString
								...
								try {
									ConceptReferenceTerm refTerm = cs
											.getConceptReferenceTermByCode(t,
													source);
						...
						default:
							columnNumber++;
							log.error(ui
									.message("conceptmanagementapps.file.line.error")
									+ columnNumber);
							break;
	...
	private FileDownload writeToFile(ArrayList<String> lines, String sourceId) {
		...
		String errorFilename = "conceptsMissingMappingsErrors" + s + "_"
				+ sourceId + ".csv";
